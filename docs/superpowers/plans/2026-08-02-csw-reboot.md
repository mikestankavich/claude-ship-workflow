...
Expected: FAIL — `cleanup: runs the sweep` and the rest, `30 passed, 10 failed` (30 from the
`work` and `merge` assertions already in place; all 10 new cleanup assertions fail because
...
Expected: PASS across every test file. `test-skills.sh` reports `47 passed, 0 failed`.
